      ...
      <div className="relative grid min-h-[22rem] overflow-hidden p-5 sm:min-h-[25rem] sm:p-8 lg:grid-cols-[minmax(0,1fr)_16rem] lg:p-10">
        <AnimatePresence mode="wait" initial={false}>
          <motion.div key={activeIndex} initial={reduceMotion ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={reduceMotion ? undefined : { opacity: 0, y: -10 }} transition={{ duration: 0.22, ease: "easeOut" }} className="relative z-10 flex max-w-2xl flex-col items-start">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/62">{activePromo.eyebrow}</p>
            ...
            {isContactPromo ? <WhatsAppButton label={activePromo.cta} variant="ghost" size="default" className="mt-7 h-auto rounded-none border-b border-white/45 px-0 pb-2 text-white hover:bg-transparent hover:text-white" /> : <a href="#collection" className="mt-7 inline-flex items-center gap-2 border-b border-white/45 pb-2 text-sm font-semibold text-white transition hover:border-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white">{activePromo.cta}<ArrowUpRight className="size-4" /></a>}
          </motion.div>
        </AnimatePresence>
        ...
        <div className="absolute bottom-5 left-5 right-5 z-10 flex items-center justify-between sm:bottom-8 sm:left-8 sm:right-8 lg:left-10 lg:right-10">
          <p className="text-[11px] font-semibold tracking-[0.18em] text-white/58" aria-live="polite">0{activeIndex + 1} / 0{promos.length}</p>
