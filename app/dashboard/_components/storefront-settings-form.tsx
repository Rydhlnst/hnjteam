...
export function StorefrontSettingsForm({ initialSettings }: StorefrontSettingsFormProps) {
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => { const formData = new FormData(); for (const [name, fieldValue] of Object.entries(value)) formData.set(name, fieldValue); startTransition(() => formAction(formData)); },
  });
