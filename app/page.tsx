  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] text-[#171716]">
      ...
      <main className="mx-auto max-w-[1440px] px-4 pb-28 pt-4 sm:px-6 sm:pb-20 sm:pt-6 lg:px-8">
        <FadeIn delay={0.05}><PromoBanner settings={settings} /></FadeIn>
        ...
        <section id="collection" className="scroll-mt-24 pt-10 sm:pt-12">
          ...
          {products.length ? (
            <FadeInStagger className="grid grid-cols-2 gap-3 sm:grid-cols-2 sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">
              {products.map((product) => <FadeInItem key={product.id}><ProductCard product={product} /></FadeInItem>)}
            </FadeInStagger>
          ) : (
            <div className="rounded-2xl border border-dashed border-[#cfcfca] bg-white px-6 py-16 text-center text-sm text-[#777773]">New products are being prepared. Check back soon.</div>
          )}
        </section>
      </main>
      <SiteFooter />
