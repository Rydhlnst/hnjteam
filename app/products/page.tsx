  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] text-[#171716]">
      ...
      <main className="mx-auto max-w-[1440px] px-4 pb-28 pt-8 sm:px-6 sm:pb-20 sm:pt-12 lg:px-8">
        ...
        {filteredProducts.length ? <section className="pt-10 sm:pt-12"><div className="mb-6 flex items-end justify-between gap-4"><p className="text-sm text-[#777773]">Showing <span className="font-semibold text-[#171716]">{filteredProducts.length}</span> {filteredProducts.length === 1 ? "product" : "products"}{query ? <> for &quot;{query}&quot;</> : null}</p><p className="hidden text-xs text-[#999995] sm:block">Updated monthly</p></div><div className="grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-3 xl:grid-cols-4">{filteredProducts.map((product) => <ProductCard key={product.id} product={product} />)}</div></section> : <div className="mt-10 rounded-2xl border border-dashed border-[#cfcfca] bg-white px-6 py-20 text-center"><div className="mx-auto max-w-sm"><p className="text-lg font-semibold text-[#171716]">No products found.</p><p className="mt-2 text-sm leading-6 text-[#777773]">Try another search or category.</p><Button asChild variant="outline" className="mt-6"><Link href="/products">Clear filters <ArrowRight className="size-4" /></Link></Button></div></div>}
      </main>
      <SiteFooter />
    </div>
  );
